...
//    canv = float4(float(0.93 + 0.07 * canv).xxx, 1.0);
    ...
    int NumGrid = int(float(0x10000 / 2.5) * min(pow(RES.x / 1920.0, 5.23), 1.0) * (1.0 - layerScale * layerScale));
    //NumGrid = 255;
    ...
    for (int layer = 11; layer >= 0; layer--)
    {
        ...
        for (int ni = 0; ni < 25; ni++) //nine near grids
        {
			...
			if (pos.x>=0 && pos.x<=1 && pos.y>=0 && pos.y<=1)
			{
				...
				float mask = 1 - pos_.y;
				mask *= max(
					pow( (cos(pos_.x * 3.14159 * 2 + 1.5 * (noise(gridsumn).x-.5))+1.)/2., 1.2) - 0.135, 
					0.35 * exp(-pos_.y * pos_.y * 5) * (1. - pos_.y)
				);
				mask = clamp(mask+0.6, 0., 1.);
			
				s = s * 0.43 + s_ * mask * 0.55;
				s = pow(s, 1.2)  * pow(1.285 - pos_.y, 0.8) - .25 * pos_.y;
				s = clamp(s,0.,1.);
			}
